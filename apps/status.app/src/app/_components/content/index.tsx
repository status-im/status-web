...
import { Image as CloudinaryImage } from '../assets'
...
import type { ImageId } from '../assets'
...
export const jobsComponents = {
  ...
  h2: (props: ComponentProps<'h2'>) => {
    return (
      <>
        <SpaceDivider className="h-14 first-of-type:hidden" />
        {baseComponents.h2({ ...props, mb: 'mb-4', mt: 'mt-6' })}
      </>
    )
  },
  span: (props: ComponentProps<'span'>) => {
    return <>{renderText({ children: props.children })}</>
  },
}
...
// todo?: reuse helpComponents
export const specsComponents = {
  ...baseComponents,
  // note: heading overriding is only temporary until Vac dis/approves change in source and template
  h1: (props: ComponentProps<'h1'>) => {
    return (
      <>
        <SpaceDivider />
  ...
  h2: (props: ComponentProps<'h2'>) => {
    return baseComponents.h3({ ...props })
  },
  h3: (props: ComponentProps<'h3'>) => {
    return baseComponents.h4({ ...props })
  },
  h4: (props: ComponentProps<'h4'>) => {
    return baseComponents.h5({ ...props })
  },
  h5: (props: ComponentProps<'h5'>) => {
    return baseComponents.h6({ ...props })
  },
  strong: (props: Parameters<typeof baseComponents.strong>[0]) => {
    return baseComponents.strong({ ...props, size: 15 })
  },
  del: (props: Parameters<typeof baseComponents.del>[0]) => {
    return baseComponents.del({ ...props, size: 15 })
  },
  em: (props: Parameters<typeof baseComponents.em>[0]) => {
    return baseComponents.em({ ...props, size: 15 })
  },
  ...
  Image: (props: { alt: string; src: string; id?: ImageId }) => {
    if (props.id) {
      return (
        <div className="[&>img]:my-5 [&>img]:w-full [&>img]:rounded-20">
          <CloudinaryImage id={props.id} alt="" />
        </div>
      )
    }

    return (
      // note: next/image requires size which would have to be set in markdown content, thus adding complexity for maintainers
      ...
      //   />
      // </div>
      <img alt={props.alt} src={props.src} className="my-5 rounded-20" />
    )
  },
}
...
export const helpComponents = {
  ...
  Admonition: (props: any) => {
    const { type, status } = props

    return (
      <div className="my-5">
        <Admonition type={type} status={status}>
          {props.children.props?.children || props.children}
        </Admonition>
      </div>
    )
  },
  Tabs: (props: React.ComponentPropsWithoutRef<typeof Tabs.Root>) => {
    return (
      <div className="mb-5 mt-1">
        <Tabs.Root {...props} variant="grey" />
      </div>
    )
  },
  TabsContent: (props: React.ComponentPropsWithoutRef<typeof Tabs.Content>) => {
    return <Tabs.Content {...props} />
  },
  TabsList: (props: React.ComponentPropsWithoutRef<typeof Tabs.List>) => {
    return (
      <div className="lg:overflow-x-hidden">
        <div className="-mr-8 mb-5 flex overflow-x-scroll pr-12 scrollbar-none">
          <Tabs.List {...props} />
        </div>
      </div>
    )
  ...
  TabsTrigger: (
    props: React.ComponentPropsWithoutRef<typeof Tabs.Trigger> & {
      children: React.ReactComponentElement<'p', { children: string }>
    }
  ) => {
    if (props.disabled) {
      return (
  ...
  TableHeader: (props: React.ComponentProps<typeof TableHeader>) => {
    return <TableHeader {...props} />
  },
  TableContent: (props: React.ComponentProps<typeof TableContent>) => {
    return <TableContent {...props} />
  },
  TableRow: (props: React.ComponentProps<typeof TableRow>) => {
    return <TableRow {...props} />
  },
  TableCell: (props: React.ComponentProps<typeof TableCell>) => {
    return <TableCell {...props} />
  },
  Divider: () => {
    return <SpaceDivider />
  },
  Step: (props: React.ComponentProps<typeof Step>) => {
    return <Step {...props} />
  },
  ContextTag: (props: { icon?: JSX.Element; children: string }) => {
    if (props.icon) {
      return (
        <span className="inline-flex align-middle">
          <ContextTag
...
export const adminComponents = {
  ...
  del: (props: Parameters<typeof baseComponents.del>[0]) => {
    return baseComponents.del({ ...props, size: 13 })
  },
  em: (props: Parameters<typeof baseComponents.em>[0]) => {
    return baseComponents.em({ ...props, size: 13 })
  },
  a: (props: Parameters<typeof baseComponents.a>[0]) => {
    return baseComponents.a({ ...props, size: 13 })
  },
  p: (props: Parameters<typeof baseComponents.p>[0]) => {
    return baseComponents.p({ ...props, size: props.size ?? 13 })
  },
  blockquote: (props: ComponentProps<'blockquote'>) => {
    return baseComponents.blockquote({ ...props, size: 19 })
  },
  ol: (props: any & { parent?: string }) => {
    ...
    return (
      <ol
        className="flex flex-col gap-[10px] [.root-content>&]:mb-5 [.root-content>span>&]:mb-5"
        {...props}
      >
        {Children.map(listItems, (item: any, index) =>
          cloneElement(item, {
  ...
  ul: (props: ComponentProps<'ul'>) => {
    return (
      <ul className="flex flex-col gap-[6px] [.root-content>&]:mb-5 [.root-content>span>&]:mb-5">
        {props.children}
      </ul>
    )
  },
