...
const IconButton = forwardRef<
  HTMLButtonElement,
  SegmentButtonProps & { icon: React.ReactNode }
>(({ children, icon, onClick, active, size = 32 }, ref) => {
  const iconWithCurrentColor = cloneElement(
...
const SegmentedControl = <T extends string | number>(props: Props<T>) => {
  ...
  useEffect(() => {
    const activeSegmentIndex = Children.toArray(children).findIndex(
      child =>
        isValidElement(child) && (child as any).props.children === activeSegment
    )
  ...
  const clonedChildren = Children.map(
    ...
    (child, index) =>
      ...
      cloneElement(child, {
        ...(child.props as any),
        onClick: () => onSegmentChange((child.props as any).children),
        active: (child.props as any).children === activeSegment,
