...
export function BlogSearch(props: Props) {
  ...
  const liveResultPosts = useMemo(() => {
    ...
    const index = loadBlogSearchIndex(searchPayload.searchIndex)
    const postsById = new Map(searchPayload.posts.map(post => [post.id, post]))

    return getBlogSearchResultIds(
      index,
      searchPayload.records,
      query,
      category
    ).flatMap(id => {
        ...
        <div aria-live="polite">
          <div className="mb-5 flex items-center justify-between gap-4">
            <Text size={15} color="$neutral-50">
              {isSearchLoading
                ? t('searchLoading')
                : t('resultCount', { count: resultPosts.length })}
            </Text>
          ...
          {isSearchLoading ? (
            ...
            <PostGrid posts={resultPosts} isLoading={false} />
          ) : (
            <div className="flex min-h-[320px] flex-col items-center justify-center text-center">
              <Image
                alt=""
