...
export function NewsTag({ post, isRelease }: Props) {
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="inline-flex h-8 select-none items-center gap-2 rounded-[56px] border border-white-10 bg-white-5 p-1 pr-[6px]"
    >
      {isRelease && (
        <span
