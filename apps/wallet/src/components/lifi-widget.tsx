...
const StatusLifiWidget = ({ config = {} }: Props) => {
    ...
    if (window.ethereum && isConnected && address) {
      setTimeout(() => {
        if (window.ethereum.emit) {
          window.ethereum.emit('connect', { chainId })
        ...
        window.dispatchEvent(
          new CustomEvent('eip6963:announceProvider', {
            detail: {
              ...
              provider: window.ethereum as unknown,
            },
          }),
        )
      }, 100)
...
export { StatusLifiWidget as LifiWidget, type WidgetConfig }
