...
const getCurrentWalletId = (): string => {
  return (window as unknown as { __statusWallet_currentWalletId?: string }).__statusWallet_currentWalletId || ''
}
...
const handleSendTransaction = async (
  ...
  const password = promptUserPassword('Enter your wallet password to sign the transaction:')
  if (!password) {
    throw new Error('Transaction cancelled: Password is required to sign transaction')
  }
...
const handleSignTypedData = async (
  ...
  const password = promptUserPassword('Enter your wallet password to sign data:')
  if (!password) {
      ...
      try {
        switch (args.method) {
          ...
          case 'eth_sendTransaction': {
            ...
            setTimeout(() => provider.emit('accountsChanged', [walletAddress]), PROVIDER_SETUP_DELAY)
            return txHash
          }

          case 'eth_signTypedData_v4': {
            if (!walletAddress) throw new Error('No wallet connected')
            return handleSignTypedData(args.params || [], walletAddress, apiClient)
          }

          case 'wallet_switchEthereumChain': {
            const result = handleSwitchChain(args.params || [], currentChainId, provider)
        ...
        throw new Error(message.includes('Transaction failed') ? message : `${args.method} failed: ${message}`)
      }
    },
...
const emitProviderEvents = (provider: EthereumProvider, address: string, chainId: number) => {
...
export function StatusAutoConnect() {
  ...
  useEffect(() => {
    ...
    const initialProvider = createEthereumProvider(undefined, chainId || DEFAULT_CHAIN_ID, apiClient)
  ...
  useEffect(() => {
    ...
    const updatedProvider = statusWallet.createEthereumProvider(address, chainId || DEFAULT_CHAIN_ID)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ;(window as any).ethereum = updatedProvider

    // Store current wallet ID for transaction signing
    ;(window as unknown as { __statusWallet_currentWalletId?: string }).__statusWallet_currentWalletId = currentWallet?.id

    const emitEvents = () => emitProviderEvents(updatedProvider, address, chainId || DEFAULT_CHAIN_ID)
    emitEvents()
  ...
  }, [currentWallet?.activeAccounts, currentWallet?.id, isConnected, connectors, connect, chainId])
