...
/**
 * Anvil fixture — wallet/on-chain tests against local Anvil forks.
 *
 * Provides a MetaMask wallet connected to the Hub, with both the extension's
 * internal RPC and the Hub frontend RPC redirected to local Anvil forks, plus
 * an AnvilRpcHelper for funding and snapshot/revert isolation. Feature-specific
 * setup (e.g. funding vault tokens) belongs in the consuming test, not here.
 *
 * Lifecycle: first test snapshots base state → each test reverts → re-snapshots
 * → funds ETH → runs.
 *
...
export const test = walletTest.extend<AnvilFixtures>({
  ...
  anvilRpc: async ({}, use, testInfo) => {
    ...
    if (!env.ANVIL_MAINNET_RPC || !env.ANVIL_LINEA_RPC) {
      throw new Error(
        'ANVIL_MAINNET_RPC and ANVIL_LINEA_RPC must be set for Anvil-based tests. ' +
    ...
    if (!baseSnapshots) {
      ...
      try {
        ...
        // Re-fund ETH on both forks. Feature-specific token balances are the
        // consuming test's responsibility, so nothing to reset here.
        await Promise.all([
          helper.setEthBalance(ANVIL_FUND_ETH_WEI),
          helper.setEthBalance(ANVIL_FUND_ETH_WEI, helper.lineaRpc),
        ])
    ...
    // multi-step flows (e.g. approve → action) pending with null receipt
