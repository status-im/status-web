  ...
  for (const vault of Object.values(TEST_VAULTS)) {
    test(
      ...
      async ({ hubPage, metamask }) => {
        ...
        await test.step(
          'Switch MetaMask to Status Network Sepolia (wrong network for all vaults)',
          async () => {
        ...
        await test.step(
          `Open deposit modal for ${vault.name}`,
          async () => {
        ...
        await test.step(
          'Verify "Switch Network to Deposit" is visible',
          async () => {
        ...
        await test.step(
          'Verify button changes to "Enter amount"',
          async () => {
