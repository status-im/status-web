...
export function useMessenger(chatId: string, chatIdList: string[]) {
  ...
  const addNewMessage = useCallback((sender: Uint8Array, content: string, date: Date, id: string) => {
    let signer = '0x';
    sender.forEach(e => {
    ...
    const createMessenger = async () => {
      ...
      await Promise.all(
        chatIdList.map(async id => {
          ...
          if (chat) {
            const messages = await messenger.waku.store.queryHistory([chat.contentTopic], {
              decryptionKeys: [chat.symKey],
            });
            messages.sort((a, b) => ((a?.timestamp?.getTime() ?? 0) > (b?.timestamp?.getTime() ?? 0) ? 1 : -1));
            messages.forEach(message => {
              if (message.payload) {
                ...
                if (metadata.payload) {
                  ...
                  if (metadata.signer && chatMessage.text && chatMessage.proto.timestamp) {
                    addNewMessage(metadata.signer, chatMessage.text, new Date(chatMessage.proto.timestamp ?? 0), id);
                  }
                }
              }
              return undefined;
  ...
  const sendMessage = useCallback(
    async (messageText: string) => {
      ...
      addNewMessage(messenger?.identity.publicKey ?? new Uint8Array(), messageText, new Date(), chatId);
    },
    [chatId, messenger]
  );
